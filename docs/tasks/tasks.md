...
1. Implement the `fx::FilePlayer` class capable of playing back audio files.
    ...
    1. Play back samples in the `processBlock()` member function.
        ...
        1. Fill the playback channels not present in the file with silence.
...
1. Open the DAW of your choice (e.g., Reaper) and see if you can add the plugin to a track.
...
1. In the `PluginEditor::resized()` method, set the slider's bounds according to the Figma design.
...
As you can see in the Figma design, we have a label indicating the slider's value. The value label should update on every slider change to reflect the current value. In JUCE, the standard way to achieve this is by observing the slider value. 
...
So far, our labels use the stock Font that ships with JUCE. That's fine for hobby projects, but professional-grade audio plugins always use custom fonts in their GUIs. Since we don't know whether the chosen font is present on the user's machine, we must bundle the fonts with the plugin binary. As you can see, we have two fonts in the Figma projects, and both are preset as TTF files in the _audio_plugin/assets/_ folder. Your task is to add them to the project and use them in the GUI.

1. Find where we define the plugin's binary data in _CMakeLists.txt_. Add both TTF files as binary data. Regenerate and recompile the project.
...
Based on the Figma design file, fill the background with programmatically generated "noise." For this, look up Figma noise parameters and implement the `drawNoise()` function in _PluginProcessor.cpp_ accordingly. Good luck!
